...
export class __CLASS_NAME__ extends API {
  ...
  /**
   * Get a paginated list of __ASSOCIATED_RESOURCE_TYPE_PASCALCASE__ IDs
   ...
   */
  async paginated__ASSOCIATED_RESOURCE_TYPE_PASCALCASE__IdList({exclusiveStartKey, size}, __RESOURCE_TYPE_CAMELCASE__Id, filter) {
    const qs = {
    ...
    const ____ASSOCIATED_RESOURCE_TYPE_CAMELCASE__Ids = response.data.map((item) => item.__ASSOCIATED_RESOURCE_TYPE_SNAKECASE___id);

    const ____ASSOCIATED_RESOURCE_TYPE_CAMELCASE__EdgePayloads = response.data.map((item) => {
      return {
