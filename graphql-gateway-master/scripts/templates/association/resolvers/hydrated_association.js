    ...
    __PREFIXED_ASSOCIATED_RESOURCE_TYPE_CAMELCASE_PLURALIZED__: async ({__internalId, __cacheHint}, {first, after, filter}, {dataSources}, info) => {
      ...
      const loadConnection = loadSqlBasedConnectionFields({first, after}, async (legacyPagination) => {
        const {
          ...
          totalCount,
        } = await dataSources.__ASSOCIATION_MICROSERVICE_PASCALCASE__.__CLASS_NAME_CAMELCASE__.paginated__ASSOCIATED_RESOURCE_TYPE_PASCALCASE__List(
            legacyPagination,
            __internalId,
            // filter,
        );
        ...
        return {
          nodes: ____ASSOCIATED_RESOURCE_TYPE_CAMELCASE_PLURALIZED__,
