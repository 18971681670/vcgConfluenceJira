...
export const resolvers = {
  UserStatsAggregation: {
    followedByUsers: async ({from, to, __cacheHint}, {first, after}, {dataSources}, info) => {
      ...
      const loadConnection = loadSqlBasedConnectionFields({first, after}, async (legacyPagination) => {
        const {
          ...
          __userEdgePayloads,
          totalCount,
        } = await dataSources.Stats.followedByUsersOnUserStatsAggregation.paginatedUserIdList(
            legacyPagination,
            from,
            to,
        );
