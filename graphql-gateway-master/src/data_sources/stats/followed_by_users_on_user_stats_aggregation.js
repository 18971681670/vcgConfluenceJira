 ...
 */
export class FollowedByUsersOnUserStatsAggregation extends API {
  /**
  ...
  async paginatedUserIdList({pageNum, pageSize}, from, to) {
    ...
    const response = await this.get(`internal/graphql/me/userStats/followerCount/timeAggregation/users`, this.tidyQuery(qs));
    const __userIds = response.followers.map((item)=>item.follower_id);
    const __userEdgePayloads = response.followers.map((item) => {
      return {
